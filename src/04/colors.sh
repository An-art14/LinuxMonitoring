...
fond_one=$column1_background
print_one=$column1_font_color
fond_two=$column2_background
print_two=$column2_font_color

if [[ -z $fond_one ]]; then
    fond_one=${b_black}
elif [[ -z $print_one ]]; then
    print_one=${f_white}
elif [[ -z $fond_two ]]; then
    fond_two=${b_red}
elif [[ -z $print_two ]]; then
    print_two=${f_blue}
fi
